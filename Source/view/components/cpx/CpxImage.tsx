...
const changeBrightness = (filterID: string, brightness: number): void => {
	const brightnessFilter: HTMLElement | null =
		window.document.getElementById(filterID);
	if (brightnessFilter) {
...
const setupSwitch = (props: IProps): void => {
	...
	if (switchElement && swInnerElement && swHousingElement) {
		...
		const svgSwitchInner: SVGElement =
			swInnerElement as unknown as SVGElement;
		const svgSwitchHousing: SVGElement =
			swHousingElement as unknown as SVGElement;
		...
		svgSwitchHousing.onmouseup = (e) =>
			props.onMouseUp(swHousingElement, e);
